  ...
  if (categoryId) {
    ...
    if (configured) {
      logger.warn(
        ...
        "Configured shared ticket category ID is not a category",
      );
    }
  }
...
async function getNextTicketNumber(guild: any, prefix: string): Promise<string> {
  // Refresh the cache so tickets created before a bot restart are included.
  await guild.channels.fetch().catch((err: unknown) => {
    logger.warn({ err: String(err) }, "Could not refresh channels while allocating ticket number");
  });

  const pattern = new RegExp(`^${prefix}-(\\d{3,})import {
  Client,
  GatewayIntentBits,
  MessageFlags,
  TextChannel,
